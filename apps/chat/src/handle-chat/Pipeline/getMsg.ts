...
export type MessageCorePipelineMode = 'full' | 'delta';

export type BuildMessageCorePipelineOptions = {
  mode?: MessageCorePipelineMode;
  /** When clear_before is applied in the outer $match (GetRooms delta path). */
  omitRoomClear?: boolean;
};

export function buildMessageCorePipeline(
  userId: string,
  opts?: BuildMessageCorePipelineOptions,
): PipelineStage[] {
  const uid = Utils.convertToObjectIdMongoose(userId);
  const isDelta = (opts?.mode ?? 'full') === 'delta';
  const omitRoomClear = opts?.omitRoomClear === true;

  const roomAndClearStages: PipelineStage[] = omitRoomClear
    ? [
        {
          $lookup: {
            from: 'Rooms',
  ...
  const attachmentStages: PipelineStage[] = isDelta
    ? [
        {
          $lookup: {
            from: 'Attachments',
        ...
        { $addFields: { attachments: { $ifNull: ['$attachments', []] } } },
      ]
    : [
        {
          $lookup: {
            ...
            pipeline: [
              ...
              },
              {
                $addFields: {
                  summary: {
                    $ifNull: [{ $first: '$ai_summary_doc.text' }, null],
                  },
                },
              },
              {
  ...
  const heavyEnrichmentStages: PipelineStage[] = isDelta
    ? []
    : [
        {
          $lookup: {
        ...
        { $addFields: { quizDoc: { $first: '$quizDoc' } } },
        {
          $lookup: {
            from: 'FlashcardDecks',
            localField: 'desk_id',
  ...
  const stages: PipelineStage[] = [
    ...
    /** 7) Project */
    {
      $project: {
        ...
        read_by: isDelta ? { $literal: [] } : '$read_list',
        read_by_count: isDelta ? { $literal: 0 } : { $size: '$read_list' },
        call_history: isDelta ? { $literal: null } : '$callHistoryDoc',
