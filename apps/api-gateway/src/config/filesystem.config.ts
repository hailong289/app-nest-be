...
export default registerAs('filesystem', () => {
  const nodeEnv =
    process.env.GATEWAY_FILESYSTEM_NODE_ENV || process.env.NODE_ENV || 'local';
  ...
  return {
    host: get('GATEWAY_FILESYSTEM_HOST') || 'localhost',
    port: get('GATEWAY_FILESYSTEM_PORT') || '5002',
    protoPath: [
      get('GATEWAY_FILESYSTEM_PROTO_PATH') || 'libs/grpc/filesystem.proto',
      get('GATEWAY_DOCUMENT_PROTO_PATH') || 'libs/grpc/document.proto',
    ],
    nodeEnv,
  };
});
