...
export default registerAs('notificationGrpc', () => {
  const nodeEnv = process.env.GATEWAY_NOTIFICATION_NODE_ENV || process.env.NODE_ENV || 'local';
  ...
  return {
    host: serviceEnv.GATEWAY_NOTIFICATION_HOST || 'localhost',
    port: serviceEnv.GATEWAY_NOTIFICATION_PORT || '5005',
    protoPath:
      serviceEnv.GATEWAY_NOTIFICATION_PROTO_PATH ||
