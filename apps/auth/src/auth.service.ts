import { LoginDto, RegisterDto, UpdateAvatarDto, UpdateProfileDto } from '@app/dto';
...
export class AuthService {
  ...
  async updateAvatar(data: UpdateAvatarDto) {
    ...
    try {
      ...
      const blob = new Blob([new Uint8Array(data.file.buffer)], { type: data.file.mimetype });
      formData.append('file', blob, data.file.originalname);
      formData.append('folder', `${data.folder}/${user.usr_id}`);
      const result = await axios.post(`${this.gatewayUrl}/api/filesystem/upload-single`, formData, {
        headers: {
      ...
      await user.save();
      return Response.success({ url: result.data.metadata.url }, 'Cập nhật ảnh đại diện thành công');
    } catch (error) {
      console.error('Error updating avatar:', error);
      return Response.error('Cập nhật ảnh đại diện thất bại', 400, 'ERROR_UPDATE_AVATAR', error);
    }
  }
